...
exports.createUser = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const userRecord = await admin.auth().createUser({
      email: email,
      password: password,
      emailVerified: false, // You can set this to true if email is verified
      // The username is the same as the email in this case
    });

    const applicationRef = await db.collection('users').doc({email}).add({
      name: "",
      skills: [],
    });
    ...
    res.status(200).send({ userId: userRecord.uid, message: 'User created successfully' });
  } catch (error) {
